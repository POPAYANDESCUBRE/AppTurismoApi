...
class LugarTuristicoViewset(viewsets.ModelViewSet):
    ...
    def valoraciones(self, request, pk=None):
        ...
        if request.method == 'GET':
            ...
            if ValoracionComentario.objects.filter(content_type=ct, object_id=lugar.id, usuario=request.user, estado=True).exists():
                return Response({"detail": "Ya has valorado este lugar."}, status=status.HTTP_400_BAD_REQUEST)
