...
from apps.restaurantes.models import Restaurante, Plato, CategoriaMenu
from apps.gestiones.models import ValoracionComentario
from .serializers import RestauranteSerializer, PlatoSerializer, CategoriaMenuSerializer
...
class RestauranteViewset(viewsets.ModelViewSet):
    ...
    def valoraciones(self, request, pk=None):
        ...
        if request.method == 'GET':
            ...
            if ValoracionComentario.objects.filter(content_type=ct, object_id=restaurante.id, usuario=request.user, estado=True).exists():
                return Response({"detail": "Ya has valorado este restaurante."}, status=status.HTTP_400_BAD_REQUEST)
